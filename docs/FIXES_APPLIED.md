...
4. **Department split-brain** (`api/security.py` create/delete department + `database/services/department_service.py`) — the Security UI's `POST/DELETE /departments` wrote **only** to in-memory state, while process routing / identity resolution read the **DB**. So a department created from the Security UI was invisible to approval routing. Now `create_department` calls `DepartmentService.save_department(...)` and `delete_department` calls a new `DepartmentService.delete_department(...)` (mirrors how `create_group` already persisted). One source of truth = the DB.
