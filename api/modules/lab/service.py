...
"""
    ...
    async def _create_document_image(
        ...
        if items:
            ...
            if items:
                cols = list(items[0].keys())[:4]
                col_width = (width - 2 * margin) // len(cols)
                
                for i, col in enumerate(cols):
                    draw.text((margin + 10 + i * col_width, y + 8), col.upper()[:15], font=font_small, fill='black')
            ...
            for item in items[:8]:
                ...
                for i, val in enumerate(values):
                    ...
                    # Try to extract numeric for total
                    if i == len(values) - 1:
                        try:
            ...
            if total > 0:
                y += 15
                draw.rectangle([width - 250, y, width - margin, y + 40], fill=header_color)
    ...
    async def _generate_structured_image_content(cls, description: str, document_type: str) -> Dict[str, Any]:
        ...
        prompt = f"""Based on this description, generate realistic document data:
...
{{
    "title": "Document title (e.g., 'AIRLINE TICKET', 'INVOICE', 'BOARDING PASS')",
    "subtitle": "Optional subtitle or company name",
    "reference": "Reference/ticket/invoice number",
    "header_color": "Hex color for header (e.g., '#1e3a5f', '#0066cc')",
    "sections": [
        {{"title": "Section name", "lines": ["line 1", "line 2"]}},
    ],
    "items": [
        {{"column1": "value1", "column2": "value2", "column3": "value3", "column4": "value4"}}
    ],
    "notes": ["Note 1", "Note 2"],
    "footer": "Footer text or company info"
}}

Make the data realistic and matching the description. For airline tickets, include flight details, passenger info, etc.
    ...
    def _get_fallback_structure(cls, description: str, document_type: str) -> Dict[str, Any]:
        ...
        if 'airline' in desc_lower or 'flight' in desc_lower or 'ticket' in desc_lower:
            return {
                ...
                "items": [
                    {"Segment": "NYC-LAX", "Class": "Economy", "Status": "Confirmed", "Fare": "$450.00"},
                    {"Segment": "Taxes", "Class": "-", "Status": "-", "Fare": "$65.00"}
                ],
            ...
            return {
                ...
                "items": [
                    {"Item": "Product 1", "Qty": "1", "Price": "$29.99", "Total": "$29.99"},
                    {"Item": "Product 2", "Qty": "2", "Price": "$15.00", "Total": "$30.00"}
                ],
