...
try:
    ...
    from api.security import router as security_router, get_current_user, get_current_user_optional, require_super_admin
...
def build_tool_definitions(tools: List['ToolConfiguration']) -> List[Dict]:
    ...
    for tool in tools:
        ...
        elif tool.type == 'create_document':
            func_name = make_unique_name("create_document", tool.id)
            tool_defs.append({
                "type": "function",
                "function": {
                    "name": func_name,
                    "description": (
                        f"Generate a downloadable document using {tool.name}. {tool.description or ''} "
                        "Choose format: 'docx' (Word), 'xlsx' (Excel), or 'pptx' (PowerPoint). "
                        "Returns a download_url for the created file."
                    ),
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "format": {
                                "type": "string",
                                "enum": ["docx", "xlsx", "pptx"],
                                "description": "docx=Word, xlsx=Excel, pptx=PowerPoint"
                            },
                            "title": {"type": "string", "description": "Document title / file name"},
                            "content": {
                                "type": "string",
                                "description": (
                                    "The body. docx: plain text or markdown (use #, ##, - for headings/bullets). "
                                    "xlsx: comma-separated rows, one row per line. "
                                    "pptx: separate slides with a blank line; first line of each slide is its heading, the rest are bullets."
                                )
                            }
                        },
...
=== YOUR TASKS ==="""
    ...
    action_tools = [t for t in agent_tools if t.type in ['email', 'api', 'webhook', 'slack', 'websearch', 'database', 'create_document']]
...
4. "model": Choose the BEST model from the available list above. 
   ...
   Consider:
   - Agent uses tools → Choose gpt-4o, gpt-4o-mini, claude-sonnet-4, gemini-2.5-pro, etc.
   - Complex reasoning/analysis → gpt-4o, claude-sonnet-4, gemini-2.5-pro
   - High volume/simple tasks → gpt-4o-mini, gemini-2.5-flash
...
async def get_available_providers():
    ...
    provider_meta = {
        ...
        "google": {"name": "Google Gemini", "default_models": ["gemini-2.5-flash"]},
...
@app.get("/api/tool-outputs/{filename}")
async def serve_tool_output(filename: str):
    """Serve a document generated by a create_document tool."""
